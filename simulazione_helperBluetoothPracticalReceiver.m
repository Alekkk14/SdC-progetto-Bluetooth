...
% function [peakValues, peakIndices, startIndices, endIndices] = findSignalPeaksWithEdges(signal)
%     % findSignalPeaksWithEdges trova i picchi nel segnale e ne determina anche
%     % l'inizio e la fine sulla base di una soglia relativa al picco.
%     %
%     % INPUT:
%     %   signal - vettore contenente il segnale
%     %
%     % OUTPUT:
%     %   peakValues   - valori dei picchi trovati
%     %   peakIndices  - indici dei picchi nel vettore di input
%     %   startIndices - indici in cui inizia ciascun picco
%     %   endIndices   - indici in cui finisce ciascun picco
% 
%     % Trova i picchi usando findpeaks
%     [peakValues, peakIndices] = findpeaks(signal, 'MinPeakProminence', max(signal)/10);
% 
%     % Inizializza vettori per gli indici di inizio e fine picco
%     startIndices = zeros(size(peakIndices));
%     endIndices = zeros(size(peakIndices));
% 
%     % Parametro per definire l'inizio e la fine del picco: ad esempio, 50% del picco
%     thresholdFactor = 0.5;
% 
%     for i = 1:length(peakIndices)
%         peakVal = peakValues(i);
%         thr = thresholdFactor * peakVal;
%         idxPeak = peakIndices(i);
% 
%         % Cerca l'inizio: partendo dal picco, scendi indici finché il segnale
%         % scende sotto la soglia oppure raggiungi l'inizio del vettore.
%         idxStart = idxPeak;
%         while idxStart > 1 && signal(idxStart) > thr
%             idxStart = idxStart - 1;
%         end
% 
%         % Cerca la fine: partendo dal picco, scendi indici finché il segnale
%         % scende sotto la soglia oppure raggiungi la fine del vettore.
%         idxEnd = idxPeak;
%         while idxEnd < length(signal) && signal(idxEnd) > thr
%             idxEnd = idxEnd + 1;
%         end
% 
%         startIndices(i) = idxStart;
%         endIndices(i) = idxEnd;
%     end